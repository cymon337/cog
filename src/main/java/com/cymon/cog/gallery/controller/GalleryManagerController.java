...
public class GalleryManagerController {
    ...
    @PutMapping("/items")
    public ResponseEntity<ResponseDto> updateItems(@ModelAttribute ItemDto updateItem, @AuthenticationPrincipal MemberDto admin) {
        log.info("registItem ={}", updateItem);

        return ResponseEntity.ok().body(new ResponseDto(HttpStatus.OK, admin.getMemberId() +" 관리자 갤러리 아이템 변경 성공 ", galleryManagerService.updateItems(updateItem)));
    }
